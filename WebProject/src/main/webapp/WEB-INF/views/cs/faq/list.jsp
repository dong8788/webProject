		...
		<div class="table-wrapper" style="margin-left:250px;margin-top:200px;overflow-x:auto;max-width:70% ">
			...
			<div align="center">
			    	...
			    	<c:if test="${pageMaker.prev}">
			    		<a href="<c:url value='/cs/faq/${pageMaker.makeSearch(pageMaker.startPage - 1)}'/>">이전</a>&nbsp;
			    	</c:if> 
			
				    <c:forEach begin="${pageMaker.startPage}" end="${pageMaker.endPage}" var="idx">
				    	<a href="<c:url value='/cs/faq/${pageMaker.makeSearch(idx)}'/>">${idx}</a>&nbsp;
			    	</c:forEach>
			
				    <c:if test="${pageMaker.next && pageMaker.endPage > 0}">
				    	<a href="<c:url value='/cs/faq/${pageMaker.makeSearch(pageMaker.endPage + 1)}'/>">다음</a>
			   	 	</c:if> 
			  	
			</div>
			<form role="form" method="get" action="<c:url value='/cs/faq'/>">
